...
func (s *AuthService) RegisterUser(c *fiber.Ctx) error {
	...
	mail_res_err := make(chan bool)
	defer close(mail_res_err)
	...
	if <-mail_res_err {
		response_map["message"] = "Registered successfully, but failed to send OTP email"
	} else {
		response_map["message"] = "Registered successfully"
	}

	return c.Status(fiber.StatusCreated).JSON(response_map)
}
